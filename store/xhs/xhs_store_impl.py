...
from tools.google_drive import GoogleDriveUploader
from tools.notification import TelegramNotifier
...
class XhsCsvStoreImplement(AbstractStore):
    ...
    async def save_data_to_csv(self, save_item: Dict, store_type: str):
        ...
        await self.post_save_actions(save_file_name)
    ...
    async def post_save_actions(self, file_path):
        if config.ENABLE_GDRIVE_UPLOAD:
            uploader = GoogleDriveUploader(config.GDRIVE_CREDENTIALS_PATH)
            file_id = await uploader.upload_file(file_path, config.GDRIVE_FOLDER_ID)

            if config.ENABLE_TELEGRAM_NOTIFICATIONS:
                notifier = TelegramNotifier(
                    config.TELEGRAM_BOT_TOKEN, config.TELEGRAM_CHAT_ID
                )
                message = f"New file uploaded to Google Drive: {file_path}\nFile ID: {file_id}"
