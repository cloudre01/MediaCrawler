...
from tools.time_util import apply_delay
...
class XiaoHongShuClient(AbstractApiClient):
    ...
    async def get_note_by_id_from_html(
        ...
        # state = re.findall(r"window.__INITIAL_STATE__=({.*})</script>", html, re.DOTALL)
        # if state:
        #     state = state[0].replace("undefined", '""')
        # else:
        #     raise ValueError("Failed to extract initial state from HTML")
